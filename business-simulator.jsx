...
import { ChevronDown, Plus, Trash2, Download, RefreshCw } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto">
          ...
          {Object.entries(scenarios).map(([scenarioName, scenario]) => (
            <div
              ...
              className={`bg-white rounded-lg shadow overflow-hidden transition-all ${
                editingScenario === scenarioName ? 'ring-2 ring-blue-500' : ''
              }`}
            >
              {/* Scenario Header */}
              <div
        ...
        <div className="bg-white rounded-lg shadow p-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {['NPV', 'IRR', 'Breakeven', 'Final Margin'].map((metric) => (
              <div key={metric} className="border border-slate-200 rounded-lg p-4">
                <p className="text-sm font-medium text-slate-600 mb-3">{metric}</p>
                  ...
                  {['Conservative', 'Baseline', 'Upside'].map((scenario) => (
                    <div key={scenario} className="flex justify-between">
                      ...
                      <span className="text-sm font-semibold text-slate-900">
                        {metric === 'NPV'
                          ? formatCurrency(projections[scenario]?.metrics.npv)
                          : metric === 'IRR'
                          ? formatPercent(projections[scenario]?.metrics.irr)
                          : metric === 'Breakeven'
                          ? `Month ${projections[scenario]?.metrics.breakeven || 'N/A'}`
                          : formatPercent(projections[scenario]?.metrics.final_year_margin)}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {[
                  ...
                  { key: 'avg_ebitda_margin', label: 'Avg EBITDA Margin', format: 'percent' },
                  ...
                  { key: 'final_year_ebitda', label: 'Final Year EBITDA', format: 'currency' },
                  { key: 'total_cash_generated', label: 'Total Cash Generated', format: 'currency' }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <Line
                  type="monotone"
                  dataKey="Conservative_CumulativeCF"
                  stroke="#ea580c"
                  name="Conservative"
                  strokeWidth={2}
                />
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold text-slate-900 mb-4">
              Year {Math.ceil(projectionMonths / 12)} Metrics
            </h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={[
